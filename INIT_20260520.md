...
- `InputSoundMenu/InputSoundMenuApp.swift`: app entry point and menu bar extra.
- `InputSoundMenu/SoundMenuView.swift`: Sound-style popover UI.
- `InputSoundMenu/AudioInputViewModel.swift`: state management, lock behavior,
  Core Audio orchestration, and the transient HUD implementation.
- `InputSoundMenu/CoreAudioInputManager.swift`: Core Audio wrapper.
- `InputSoundMenu/InputDevice.swift`: device model and display metadata.
...
The script builds the Debug app, kills a running `InputSoundMenu` process if
